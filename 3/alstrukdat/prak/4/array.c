...
/* F.S. Tout berisi salinan Tin */
    ...
    for(int i=GetFirstIdx(Tin);i<=GetLastIdx(Tin);i++) SetEl(Tout,i,GetElmt(Tin,i));
}
...
/* Jika T kosong : Hanya menulis "Tabel kosong" */
    if(IsEmpty(T)) printf("Tabel kosong\n");
        ...
        for(int i=GetFirstIdx(T);i<=GetLastIdx(T);i++) printf("%d:%d\n",i,GetElmt(T,i));
    }
}
...
/* Mengirimkan T1 + T2 */
    TabInt T3;
    T3.Neff=NbElmt(T1);
    for(int i=GetFirstIdx(T1);i<=GetLastIdx(T1);i++) T3.TI[i]=GetElmt(T1,i)+GetElmt(T2,i);
    return T3;
}
...
/* Mengirimkan indeks i dengan elemen ke-i adalah nilai maksimum pada tabel */
    IdxType iMax=GetFirstIdx(T);
    for(int i=GetFirstIdx(T)+1;i<=GetLastIdx(T);i++) {
        if(GetElmt(T,iMax)<GetElmt(T,i)) iMax=i;
    }
    return iMax;
}
...
/* dengan elemen ke-i nilai minimum pada tabel */
    IdxType iMin=GetFirstIdx(T);
    for(int i=GetFirstIdx(T)+1;i<=GetLastIdx(T);i++) {
        if(GetElmt(T,iMin)>GetElmt(T,i)) iMin=i;
    }
    return iMin;
}